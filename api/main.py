...
TURSO_DATABASE_URL = "libsql://asasaying-jade-cyan.aws-ap-northeast-1.turso.io"
TURSO_AUTH_TOKEN = "eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3ODgxNTAxNDgsImlkIjoiMDFhMDU2MDQtOWEwMS03OWJmLWJkM2ItOTEwNGQ1NGNhODNhIiwia2lkIjoiRTNlSXNaWk0wMHRFTnF0Q3Y2NFhZeUVCZUpNcWJ3U19wTGxHaDV0bUtrTSIsInJpZCI6ImJkYjMyODBiLWYwMDgtNDViOS1iMDAxLWUwNzJlMDVkMGYwNyJ9.q9kcBymrDmZzl3WYGuw_pWHkEWhDo1-WUHag3Chqyi2GDYDXrwEMwm56NxfGrYhWNcBWPHmNO6x0C-4b_ZozBQ"
...
def get_conn():
    ...
    if _global_conn is None:
        _global_conn = turso_serverless.connect(
            TURSO_DATABASE_URL,
...
# # 临时结果容器（多数操作改为局部变量）
    ...
    def get_text():
        ...
        try:
            ...
            return row[0], row[1], row[2], row[5], row[6], row[7]
    ...
    with ui.tab_panels(tabs, value='随机浏览').classes('w-full min-h-screen bg-slate-50 py-8') as weblist:
        with ui.tab_panel('随机浏览'):
            ...
            with ui.column().classes('w-full max-w-4xl mx-auto p-8 md:p-12 rounded-3xl border border-slate-200 bg-white shadow-lg hover:shadow-2xl transition-all duration-300 items-center'):
                ...
                ui.separator().classes('my-3 w-full')
                ui.button('🎲 随缘', on_click=show_random).classes('mt-6 px-6 py-3 text-lg rounded-full shadow-md bg-gradient-to-r from-indigo-500 to-blue-500 text-white hover:scale-105 transition-transform').props('elevated')
        ...
        with ui.tab_panel('上传分享'):
            ...
            # 从数据库读取当前条目数以显示下一个 id（更可靠）
                ...
                newui = oldui + 1
                ui.label('ID：'+ str(newui)).classes('text-sm text-slate-500')
                ...
                def find():
                    ...
                    if rows:
                        ...
                        for r in rows:
                            ...
                            meta = []
                            if r[1]:
                                meta.append(f"作者：{r[1]}")
                            if r[2]:
                                meta.append(f"来源：{r[2]}")
                            if meta:
                                parts.append(t + '\n' + ' · '.join(meta))
                            else:
                                parts.append(t)
        ...
        with ui.tab_panel('关于网站'):
            ...
            with ui.column().classes('text-center w-full max-w-4xl mx-auto p-8 md:p-12 rounded-3xl border border-slate-200 bg-white shadow-lg hover:shadow-2xl transition-all duration-300 items-start gap-3'):
                ...
                ui.label("1.2026年8月28日，部署在Railway上的网页因经济原因停用").classes('text-center text-lg text-slate-800')
                ui.label("2.用turso以及vercel重新部署").classes('text-center text-lg text-slate-800')
